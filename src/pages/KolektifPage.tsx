...
export default function KolektifPage() {
  ...
  const grandTotal = sessions.reduce((sum, s) => sum + (sessionTotals[s.id] ?? 0), 0)
  ...
  return (
    <div className="grid gap-4">
      ...
      <div className="flex items-center justify-between gap-3">
        {sessions.length > 0 && (
          <div className="rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 px-5 py-3 flex items-center gap-4">
            <div>
              <div className="text-xs text-slate-500 dark:text-slate-400">Total Semua</div>
              <div className="text-lg font-bold text-emerald-600 dark:text-emerald-400">{formatIDR(grandTotal)}</div>
            </div>
            <div className="h-8 w-px bg-slate-200 dark:bg-slate-700" />
            <div>
              <div className="text-xs text-slate-500 dark:text-slate-400">Sub-buku</div>
              <div className="text-lg font-bold text-slate-900 dark:text-white">{sessions.length}</div>
            </div>
          </div>
        )}
        {userCanEdit && (
          <Button onClick={() => { setNewName(''); setOpenAddModal(true) }}>
            + Sub-buku
          </Button>
        )}
      </div>
      ...
      {sessions.length === 0 ? (
        <div className="rounded-xl border border-dashed border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 px-6 py-12 text-center">
          <div className="text-slate-400 dark:text-slate-500 text-sm">
            Belum ada sub-buku.{userCanEdit ? ' Klik "+ Sub-buku" untuk menambahkan.' : ''}
          </div>
        </div>
      ) : (
        <div className="rounded-xl border border-slate-200 dark:border-slate-700 overflow-hidden">
          {sessions.map((session, idx) => (
            ...
            >
              ...
              <button
                type="button"
                onClick={() => navigate(`/buku-kas-kolektif/${safeBookId}/sesi/${session.id}`)}
              ...
              {userCanEdit && (
                <div className="flex items-center gap-1 shrink-0">
                  <button
                    type="button"
                    onClick={() => { setRenamingSession(session); setRenameInput(session.name); setOpenRenameModal(true) }}
                  ...
                  <button
                    type="button"
                    onClick={() => { setDeletingSession(session); setOpenDeleteModal(true) }}
      ...
      <Modal open={openRenameModal} title="Rename Sub-buku" onClose={() => setOpenRenameModal(false)}>
        <div className="grid gap-4">
          <div>
            <div className="mb-1 text-xs font-medium text-slate-600 dark:text-slate-400">Nama Baru</div>
            <Input
              value={renameInput}
      ...
      <Modal open={openDeleteModal} title="Hapus Sub-buku" onClose={() => setOpenDeleteModal(false)}>
        <div className="grid gap-4">
          <p className="text-sm text-slate-600 dark:text-slate-400">
            Yakin hapus sub-buku <span className="font-semibold text-slate-900 dark:text-white">"{deletingSession?.name}"</span>? Semua data di dalamnya akan ikut terhapus.
          </p>
          <div className="flex justify-end gap-2">
